...
export default function TestSupabase() {
  const [tests, setTests] = useState<{[key: string]: 'pending' | 'success' | 'error' | 'running'}>({
    connection: 'pending',
    insert: 'pending',
    realtime: 'pending'
  })
  ...
  const updateTest = (test: string, status: 'pending' | 'success' | 'error' | 'running', result?: any) => {
  ...
  const runTests = async () => {
    ...
    try {
      ...
      updateTest('connection', 'success', `Connected! Found ${count} submissions`)
    } catch (error: any) {
